...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if ($isPgsql) {
            // 1. pengiriman_beras
            DB::statement("ALTER TABLE pengiriman_beras ALTER COLUMN status TYPE VARCHAR(100) USING status::varchar");
            DB::statement("ALTER TABLE pengiriman_beras ALTER COLUMN status SET DEFAULT 'menunggu'");

            DB::statement("ALTER TABLE pengiriman_beras ALTER COLUMN jenis_beras TYPE VARCHAR(100) USING jenis_beras::varchar");
            DB::statement("ALTER TABLE pengiriman_beras ALTER COLUMN jenis_beras SET DEFAULT 'medium'");

            // 2. hasil_pengemasan
            DB::statement("ALTER TABLE hasil_pengemasan ALTER COLUMN jenis_kemasan TYPE VARCHAR(100) USING jenis_kemasan::varchar");
            DB::statement("ALTER TABLE hasil_pengemasan ALTER COLUMN jenis_kemasan SET DEFAULT '5kg'");

            DB::statement("ALTER TABLE hasil_pengemasan ALTER COLUMN kualitas TYPE VARCHAR(100) USING kualitas::varchar");
            DB::statement("ALTER TABLE hasil_pengemasan ALTER COLUMN kualitas SET DEFAULT 'layak_jual'");
            DB::table('hasil_pengemasan')->where('kualitas', 'layak jual')->update(['kualitas' => 'layak_jual']);

            DB::statement("ALTER TABLE hasil_pengemasan ALTER COLUMN jenis_beras TYPE VARCHAR(100) USING jenis_beras::varchar");
            DB::statement("ALTER TABLE hasil_pengemasan ALTER COLUMN jenis_beras SET DEFAULT 'medium'");

            // 3. pesanan
            DB::statement("ALTER TABLE pesanan ALTER COLUMN jenis_produk TYPE VARCHAR(100) USING jenis_produk::varchar");

            DB::statement("ALTER TABLE pesanan ALTER COLUMN status TYPE VARCHAR(100) USING status::varchar");
            DB::statement("ALTER TABLE pesanan ALTER COLUMN status SET DEFAULT 'menunggu'");
            DB::table('pesanan')->where('status', 'pending')->update(['status' => 'menunggu']);

            // 4. penerimaan_beras
            DB::statement("ALTER TABLE penerimaan_beras ALTER COLUMN status TYPE VARCHAR(100) USING status::varchar");
            DB::statement("ALTER TABLE penerimaan_beras ALTER COLUMN status SET DEFAULT 'menunggu'");

            // 5. penerimaan_beras
            DB::statement("ALTER TABLE penerimaan_beras ALTER COLUMN jenis_beras TYPE VARCHAR(100) USING jenis_beras::varchar");
            DB::statement("ALTER TABLE penerimaan_beras ALTER COLUMN jenis_beras SET DEFAULT 'medium'");
        } else {
    ...
    public function down(): void
    {
        ...
        if ($isPgsql) {
            ...
            DB::statement("ALTER TABLE pengiriman_beras ALTER COLUMN jenis_beras SET DEFAULT 'medium'");
            DB::statement("ALTER TABLE hasil_pengemasan ALTER COLUMN jenis_kemasan SET DEFAULT '5kg'");
            DB::statement("ALTER TABLE hasil_pengemasan ALTER COLUMN kualitas SET DEFAULT 'layak jual'");
            DB::statement("ALTER TABLE hasil_pengemasan ALTER COLUMN jenis_beras SET DEFAULT 'medium'");
        } else {
